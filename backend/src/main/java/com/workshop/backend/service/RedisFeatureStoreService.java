...
public class RedisFeatureStoreService {
    ...
    private static final Logger log = LoggerFactory.getLogger(RedisFeatureStoreService.class);
    ...
    public void pushTransaction(Transaction txn) {
        ...
        try {
            ...
            log.warn("Redis is unavailable, skipping feature push. Reason: {}", e.getMessage());
        }
    }
    ...
    public Map<String, Object> calculateFeatures(String cardNumber, String emailDomain, LocalDateTime currentTime) {
        try {
            ...
            log.warn("Redis unavailable, falling back to database query. Reason: {}", e.getMessage());
            return calculateFeaturesFromDatabase(cardNumber, emailDomain, currentTime);
        }
    }

    private Map<String, Object> calculateFeaturesFromDatabase(String cardNumber, String emailDomain, LocalDateTime currentTime) {
        LocalDateTime timeNow = currentTime != null ? currentTime : LocalDateTime.now();
        LocalDateTime oneHourAgo = timeNow.minusHours(1);
        LocalDateTime oneDayAgo = timeNow.minusDays(1);
        LocalDateTime sevenDaysAgo = timeNow.minusDays(7);

        Map<String, Object> features = new HashMap<>();
        
        features.put("txn_count_1h", transactionRepository.countByCardNumberAndTimestampBetween(cardNumber, oneHourAgo, timeNow));
        features.put("txn_count_24h", transactionRepository.countByCardNumberAndTimestampBetween(cardNumber, oneDayAgo, timeNow));
        features.put("txn_count_7d", transactionRepository.countByCardNumberAndTimestampBetween(cardNumber, sevenDaysAgo, timeNow));

        features.put("amt_sum_1h", transactionRepository.sumAmountByCardNumberAndTimestampBetween(cardNumber, oneHourAgo, timeNow));
        features.put("amt_sum_24h", transactionRepository.sumAmountByCardNumberAndTimestampBetween(cardNumber, oneDayAgo, timeNow));
        features.put("amt_sum_7d", transactionRepository.sumAmountByCardNumberAndTimestampBetween(cardNumber, sevenDaysAgo, timeNow));

        if (emailDomain != null && !emailDomain.isEmpty()) {
            Long emailCount = transactionRepository.countByPurchaserEmailDomainAndTimestampBefore(emailDomain, timeNow);
            features.put("is_new_email", (emailCount == null || emailCount == 0) ? 1.0 : 0.0);
        } else {
            features.put("is_new_email", 0.0);
        }
